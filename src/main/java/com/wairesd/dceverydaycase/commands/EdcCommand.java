...
public class EdcCommand implements SubCommandExecutor, SubCommandTabCompleter {
    ...
    public EdcCommand(Main addon) { this.addon = addon; }

    @Override
    public boolean execute(@NotNull DCCommandSender sender, @NotNull String label, String[] args) {
        if (!(sender instanceof DCPlayer)) {
            sender.sendMessage(addon.getConfigManager().getOnlyForPlayersMessage());
            return true;
        }
        if (!sender.hasPermission("dc.everydaycase.granted")) {
            sender.sendMessage(DCTools.rc(addon.getConfigManager().getNoPermissionMessage()));
            return true;
        }
